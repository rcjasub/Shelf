...
// All variants share viewBox="0 0 310 40"; sourced from the original component's hand-drawn squiggle set.
...
// Shared across every instance on the page, mirroring the original `let nextIndex = null` module global
// so hovering across multiple links cycles through variants instead of repeating the same one.
let nextIndex: number | null = null;

function getSvgVariant(): PathData {
  if (nextIndex === null) {
    nextIndex = Math.floor(Math.random() * pathDataVariants.length);
  } else {
    nextIndex = (nextIndex + 1) % pathDataVariants.length;
  }
  return pathDataVariants[nextIndex];
}
...
export function DrawUnderline({
  ...
  return (
    <span className={cn("relative", className)} onMouseEnter={handleMouseEnter} onMouseLeave={handleMouseLeave}>
      {children}
      <span className="pointer-events-none absolute inset-x-4 -bottom-0.5 h-2.5 overflow-visible">
        {currentSvg && (
          <svg
          ...
          >
            <path ref={pathRef} d={currentSvg.d} stroke={strokeColor} strokeWidth="10" strokeLinecap="round" />
          </svg>
        )}
      </span>
    </span>
