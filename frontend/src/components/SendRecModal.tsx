                ...
                <span key={n} className="flex items-center gap-1.5">
                  <span
                    className={`flex h-5 w-5 items-center justify-center rounded-full text-[10px] font-bold ${
                      rec.step >= n ? "bg-shelf-accent text-shelf-ink" : "bg-shelf-cream/10 text-shelf-cream/40"
                    }`}
                  >
                    {n}
                  </span>
                  {n < 3 && <span className="text-[10px] text-shelf-cream/20">→</span>}
                </span>
              ))}
            </div>
          </div>
        ...
        {rec.step === 2 && (
          <div className="px-7 py-6">
            ...
            <div className="flex justify-between">
              <button
                onClick={() => selectRecBook(rec.bookId ?? books[0].id)}
                className="rounded-[3px] border border-shelf-cream/12 px-4.5 py-2.5 text-xs font-medium text-shelf-cream/50"
              >
                ← Back
              </button>
            </div>
        ...
        {rec.step === 3 && (
          <div className="px-7 py-6">
            ...
            <textarea
              ...
              className="mb-5 block h-32 w-full resize-none rounded-md border border-shelf-cream/9 bg-[#f0ebe0] p-4 font-sans text-sm leading-relaxed text-shelf-ink outline-none"
            />
            <div className="flex justify-between">
              <button
                onClick={() => selectRecFriend(rec.friendId ?? friends[0].id)}
                className="rounded-[3px] border border-shelf-cream/12 px-4.5 py-2.5 text-xs font-medium text-shelf-cream/50"
              >
                ← Back
              </button>
              <button
                onClick={submitRec}
                className="rounded-[3px] bg-shelf-cream px-6 py-2.5 text-xs font-bold uppercase tracking-wide text-shelf-bg"
              >
                Send Rec ✦
              </button>
            </div>
